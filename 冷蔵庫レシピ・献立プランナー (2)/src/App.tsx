...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 初期状態で、冷蔵庫にあるすべての食材を自動で選択（チェックON）にする
    let parsedFridgeItems: FridgeItem[] = [];
    if (savedFridge) {
      try { parsedFridgeItems = JSON.parse(savedFridge); } catch (e) { parsedFridgeItems = INITIAL_FRIDGE_ITEMS; }
    } else {
      parsedFridgeItems = INITIAL_FRIDGE_ITEMS;
    }
    setSelectedIngredientIds(parsedFridgeItems.map(item => item.id));
