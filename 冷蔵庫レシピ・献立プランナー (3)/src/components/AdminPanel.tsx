...
import { getAllUsersFromDb, syncUserToDb, deleteUserFromDb, getUserStatesFromDb, syncUserStatesToDb } from '../lib/supabase.ts';
...
export default function AdminPanel({
  ...
  useEffect(() => {
    ...
    getUserStatesFromDb(selectedUserId)
      .then((dbState) => {
        if (dbState) {
          ...
          setInspectFridge(fItems);
          setInspectShopping(sItems);
          setInspectPlanned(pMeals);
          setInspectNotifications(mNotifs);

          // Keep cache synced
          localStorage.setItem(`shufu_fridge_items_${selectedUserId}`, JSON.stringify(fItems));
          localStorage.setItem(`shufu_shopping_items_${selectedUserId}`, JSON.stringify(sItems));
          localStorage.setItem(`shufu_planned_meals_${selectedUserId}`, JSON.stringify(pMeals));
          localStorage.setItem(`shufu_notifications_${selectedUserId}`, JSON.stringify(mNotifs));
        } else {
          // Local fallback
      ...
      .catch((err) => {
        console.error("Failed to load inspect states from remote DB:", err);
        // Local fallback
        const fridge = localStorage.getItem(`shufu_fridge_items_${selectedUserId}`);
        const shopping = localStorage.getItem(`shufu_shopping_items_${selectedUserId}`);
        const planned = localStorage.getItem(`shufu_planned_meals_${selectedUserId}`);
  ...
  const saveInspectFridge = (updated: FridgeItem[]) => {
    ...
    syncUserStatesToDb(selectedUserId, { fridge_items: updated }).catch(e => console.error("Sync error:", e));
    onRefreshData?.();
  };
  ...
  const saveInspectShopping = (updated: any[]) => {
    ...
    syncUserStatesToDb(selectedUserId, { shopping_items: updated }).catch(e => console.error("Sync error:", e));
    onRefreshData?.();
  };
  ...
  const saveInspectPlanned = (updated: any[]) => {
    ...
    syncUserStatesToDb(selectedUserId, { planned_meals: updated }).catch(e => console.error("Sync error:", e));
    onRefreshData?.();
  };
  ...
  const saveInspectNotifications = (updated: any[]) => {
    ...
    syncUserStatesToDb(selectedUserId, { notifications: updated }).catch(e => console.error("Sync error:", e));
    onRefreshData?.();
  };
