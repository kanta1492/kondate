...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Load custom isolated Planned Meals
    ...
    // Load custom isolated Shopping List
    ...
    // Load custom isolated notifications logs
    ...
    // Load custom isolated Search history Database
  ...
  // Simulate notification mock triggers for real-time testing
  const handleSimulateNotification = (type: NotificationItem['type'], category: string) => {
    if (category === 'fridge') {
      const foods = ['有機キャベツ', '飛騨牛カルビ', 'こだわり低脂肪乳', '北海道たまご'];
      const food = foods[Math.floor(Math.random() * foods.length)];
      addNotification(
        '🥬 食材の自動搬入完了',
        `模擬食材［${food} 1パック］を冷蔵庫へ自動登録しました。`,
        'success'
      );
    } else if (category === 'recipe') {
      const items = ['ふわとろオムライス', '本格ビーフカレー', '時短スタミナ豚生姜焼き', '鮭のちゃんちゃん焼き'];
      const recipe = items[Math.floor(Math.random() * items.length)];
      addNotification(
        '✨ AI自動レシピ提案',
        `Geminiが『${recipe}』の時短レシピを高速提案しました。(調理時間20分/難易度：簡単)`,
        'info'
      );
    } else if (category === 'planner') {
      const dates = ['今日', '明日', '週末の日曜日'];
      const dateStr = dates[Math.floor(Math.random() * dates.length)];
      addNotification(
        '📅 献立シミュレーション確認',
        `${dateStr}の夕食メニューとして『シェフのこだわりハンバーグ』をスケジュール登録しました。`,
        'success'
      );
    } else if (category === 'expiry') {
      const foods = ['こだわり低脂肪乳', '鮮度抜群な中トロ', '無塩バター'];
      const food = foods[Math.floor(Math.random() * foods.length)];
      addNotification(
        '⌛ 食材消費期限アラート',
        `警告：冷蔵庫の［${food}］がまもなく期限切れ(あと1日)を迎えます！`,
        'warning'
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-sage-50/50 flex flex-col font-sans antialiased pb-12">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full">
        ...
        <div id="motivation-welcome-banner" className="mb-8 bg-sage-600 rounded-3xl p-6 text-white shadow-xs relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative z-10 max-w-2xl text-center md:text-left">
            <h2 className="text-lg font-bold flex items-center justify-center md:justify-start gap-2 mb-1">
              冷蔵庫の食材から、今日のメニューをスマートに
            </h2>
            <p className="text-xs text-sage-100 font-medium leading-relaxed">
          ...
          <div className="shrink-0 flex gap-2 relative z-10">
            <span className="text-xs bg-white/10 text-white border border-white/15 px-3.5 py-1.5 rounded-xl font-semibold flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5 text-sage-100" />
              手軽な時短調理
            </span>
            <span className="text-xs bg-white/10 text-white border border-white/15 px-3.5 py-1.5 rounded-xl font-semibold flex items-center gap-1.5">
              <Lightbulb className="h-3.5 w-3.5 text-sage-100" />
              代用アイデア
            </span>
          </div>
        </div>
