...
export default function MealPlanner({
  ...
  const [currentDate, setCurrentDate] = useState<Date>(new Date('2026-06-07')); // Hardcoded to prompt-based real date 2026-06-07 (or dynamic today)

  // Custom Adding State
  const [isAddingCustom, setIsAddingCustom] = useState(false);
  const [customTitle, setCustomTitle] = useState('');
  const [customDate, setCustomDate] = useState('2026-06-07');
    ...
    <div className="space-y-6" id="meal-planner-section">
      ...
      <div className="bg-white rounded-3xl p-5 shadow-xs border border-sage-100 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={() => setCurrentDate(new Date('2026-06-07'))}
        ...
        <div className="lg:col-span-3">
          <div className="bg-white rounded-3xl p-6 shadow-xs border border-sage-100">
            ...
            <div className="flex items-center justify-between border-b border-sage-50 pb-3 mb-6">
              ...
              <span className="text-xs bg-sage-50 text-sage-800 px-3 py-1 rounded-sm font-bold">期間内予定: <strong>{plannedMeals.filter(pm => weekDates.map(wd => wd.toISOString().split('T')[0]).includes(pm.date)).length}</strong>件</span>
            </div>
            ...
            <div className="space-y-6" id="planner-week-calendar">
              {weekDates.map((dateObj) => {
                ...
                // Highlight today (2026-06-07)
                const isTodayStr = dateKey === '2026-06-07';
