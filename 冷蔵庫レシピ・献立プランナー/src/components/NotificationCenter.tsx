  ...
  return (
    <div className="relative font-sans">
      ...
      <button
        ...
        className={`relative p-2 rounded-xl transition-all border cursor-pointer select-none flex items-center justify-center ${
          isOpen
      ...
      >
        {unreadCount > 0 ? (
          <BellRing className="h-4.5 w-4.5 text-inherit animate-swing" />
        ) : (
          <Bell className="h-4.5 w-4.5 text-inherit" />
        )}
        
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 bg-rose-600 text-white text-[9px] font-bold h-4 min-w-4 px-1 rounded-full flex items-center justify-center border-2 border-white animate-scale-up">
            {unreadCount}
          </span>
        )}
      </button>
      ...
      {isOpen && (
        <div 
          className="absolute right-0 mt-2.5 w-85 sm:w-100 bg-white rounded-2xl shadow-xl border border-gray-150 z-50 text-gray-850 animate-scale-up overflow-hidden"
        ...
        >
          ...
          {/* Interactive Demo Generator Controls for Testing */}
          <div className="bg-slate-50 border-t border-gray-200 p-3 select-none">
            <span className="text-[10px] font-bold text-gray-400 block mb-2 uppercase tracking-wide">💡 通知シミュレーション（動作テスト用）</span>
            <div className="grid grid-cols-4 gap-1.5">
              <button
                type="button"
                onClick={() => onSimulateNotification('success', 'fridge')}
                className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-150 py-1 rounded-lg text-[9px] font-bold transition-all cursor-pointer flex flex-col items-center justify-center gap-1.5"
              >
                <span>🧅</span>
                <span>食材登録</span>
              </button>
              <button
                type="button"
                onClick={() => onSimulateNotification('info', 'recipe')}
                className="bg-indigo-50 hover:bg-indigo-100 text-indigo-800 border border-indigo-150 py-1 rounded-lg text-[9px] font-bold transition-all cursor-pointer flex flex-col items-center justify-center gap-1.5"
              >
                <span>🍳</span>
                <span>AIレシピ</span>
              </button>
              <button
                type="button"
                onClick={() => onSimulateNotification('success', 'planner')}
                className="bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-150 py-1 rounded-lg text-[9px] font-bold transition-all cursor-pointer flex flex-col items-center justify-center gap-1.5"
              >
                <span>📅</span>
                <span>献立予約</span>
              </button>
              <button
                type="button"
                onClick={() => onSimulateNotification('warning', 'expiry')}
                className="bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-150 py-1 rounded-lg text-[9px] font-bold transition-all cursor-pointer flex flex-col items-center justify-center gap-1.5"
              >
                <span>⌛</span>
                <span>期限警告</span>
              </button>
