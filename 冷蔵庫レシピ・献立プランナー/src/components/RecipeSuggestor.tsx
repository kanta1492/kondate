...
export default function RecipeSuggestor({
  ...
  const handleGenerateRecipes = async () => {
    ...
    try {
      const selectedNamesOnly = fridgeItems
        .filter(item => selectedIngredientIds.includes(item.id))
        .map(item => item.name);
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xs border border-sage-100 kitchen-shines" id="ai-generate-settings">
        ...
        <div className="flex justify-end">
          <button
            ...
            className={`cursor-pointer px-6 py-3 rounded-2xl font-bold text-sm shadow-sm flex items-center space-x-2.5 transition-all ${
      ...
      {recipes.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" id="recipes-results-holder">
          ...
          <div className="lg:col-span-1 space-y-3">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-1">提案されたレシピリスト</h3>
