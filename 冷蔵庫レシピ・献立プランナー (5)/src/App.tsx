...
import { syncUserToDb, getAllUsersFromDb, syncUserStatesToDb, getUserStatesFromDb, migrateUserIdInDb } from './lib/supabase.ts';
...
export default function App() {
  ...
  // Keep sessionStorage in sync with current logged-in user
  useEffect(() => {
    if (currentUser) {
      sessionStorage.setItem('shufu_current_user', JSON.stringify(currentUser));
    } else {
      sessionStorage.removeItem('shufu_current_user');
    }
  }, [currentUser]);
    ...
    getAllUsersFromDb()
      .then((dbUsers) => {
        if (dbUsers && dbUsers.length > 0) {
          ...
          localUsers.forEach(u => mergedMap.set(u.id.toLowerCase(), u));
          dbUsers.forEach(u => mergedMap.set(u.id.toLowerCase(), u));
  ...
  const handleUpdateUserProfile = (updatedUser: User, oldUserId?: string) => {
    ...
    if (saved) {
      try {
        ...
        const targetSearchId = oldUserId || updatedUser.id;
        const updatedUsers = users.map(u => u.id === targetSearchId ? updatedUser : u);
    ...
    // LocalStorage keys migration if user ID is updated
    if (oldUserId && oldUserId !== updatedUser.id) {
      const prefixes = [
        'shufu_fridge_items_',
        'shufu_shopping_items_',
        'shufu_planned_meals_',
        'shufu_notifications_',
        'shufu_user_item_history_',
        'shufu_recipes_'
      ];
      prefixes.forEach(prefix => {
        const value = localStorage.getItem(prefix + oldUserId);
        if (value !== null) {
          localStorage.setItem(prefix + updatedUser.id, value);
          localStorage.removeItem(prefix + oldUserId);
        }
      });
    }
    
    // Persist to Supabase (utilize migration routine if ID has been altered)
    const syncPromise = (oldUserId && oldUserId !== updatedUser.id)
      ? migrateUserIdInDb(oldUserId, updatedUser)
      : syncUserToDb(updatedUser);

    syncPromise
      .then(() => {
        addNotification('⚙️ 設定保存', 'プロフィール設定を変更し、クラウドと同期しました。', 'success');
      })
      .catch(err => {
        console.error("Failed to sync updated profile to Supabase:", err);
        addNotification('⚠ 同期エラー', 'プロフィールの同期に失敗しました（ローカルのみ保存されました）。', 'warning');
      });
  };
