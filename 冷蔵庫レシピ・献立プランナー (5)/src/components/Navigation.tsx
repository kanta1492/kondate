...
import { ChefHat, ClipboardList, Calendar, Inbox, Sparkles, AlertCircle, LogOut, LogIn, ShieldAlert, User as UserIcon, Settings } from 'lucide-react';
    ...
    <header className="bg-white border-b border-sage-100 shadow-xs sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-2 px-3 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-center py-2.5 md:py-3.5 space-y-2 md:space-y-0">
          {/* Logo Brand */}
          <div className="flex items-center space-x-2 self-start md:self-auto select-none">
            <div 
              onClick={() => {
                // Secret tap tracker: 3 quick taps on the chef hat icon triggers admin activation
                const now = Date.now();
                const globalWindow = window as any;
                if (!globalWindow._lastLogoTap || now - globalWindow._lastLogoTap > 1200) {
                  globalWindow._logoTapCount = 1;
                } else {
                  globalWindow._logoTapCount = (globalWindow._logoTapCount || 0) + 1;
                }
                globalWindow._lastLogoTap = now;
                if (globalWindow._logoTapCount >= 3) {
                  onTriggerAdminPrivilege();
                  globalWindow._logoTapCount = 0;
                }
              }}
              className="bg-sage-600 hover:bg-sage-700 active:scale-95 text-white p-1.5 md:p-2 rounded-xl shadow-2xs flex items-center justify-center shrink-0 cursor-pointer transition-all"
              title="AI献立"
            >
          ...
          <div className="flex flex-wrap items-center gap-1.5 md:gap-2.5 text-[10.5px] md:text-xs w-full md:w-auto justify-between md:justify-end">
            ...
            {/* Auth Block */}
            <div className="flex items-center space-x-2 border-l border-sage-100 pl-2 shrink-0">
              {currentUser ? (
                <div className="flex items-center gap-1.5 align-middle">
                  {currentUser.facePhoto ? (
                    <img
                      src={currentUser.facePhoto}
                      alt="Avatar"
                      className="h-6 w-6 rounded-full object-cover border border-sage-200 shadow-3xs"
                      referrerPolicy="no-referrer"
                    />
                  ) : (
                    <div className="h-6 w-6 rounded-full bg-sage-100 flex items-center justify-center font-bold text-sage-700 text-[10px]">
                      {currentUser.name.charAt(0)}
                    </div>
                  )}
                  <div className="flex flex-col text-right">
                    <span className="font-bold text-slate-800 flex items-center gap-0.5 justify-end text-[10px] md:text-[11px] leading-tight select-none font-sans">
                      {currentUser.name}
                      {currentUser.role === 'admin' ? (
                        <span className="bg-amber-100 text-amber-800 text-[8.5px] px-1.5 py-0.5 rounded-md font-bold border border-amber-200 shrink-0 select-none">
                          管理員
                        </span>
                      ) : (
                        <span
                          onClick={onTriggerAdminPrivilege}
                          title="アカウント情報"
                          className="bg-indigo-50 text-indigo-700 text-[8.5px] px-1.5 py-0.5 rounded-md font-bold border border-indigo-100 shrink-0 cursor-default select-none"
                        >
                          会員
                        </span>
                      )}
                  ...
                  <button
                    ...
                    className="p-1.5 text-gray-400 hover:text-sage-600 hover:bg-sage-50 rounded-md transition-colors cursor-pointer shrink-0"
                    title="設定"
                    id="settings-nav-btn"
                  >
