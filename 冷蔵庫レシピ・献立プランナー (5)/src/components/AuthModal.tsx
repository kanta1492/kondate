...
export default function AuthModal({ isOpen, onClose, onLoginSuccess, fullScreenMode = false, onTriggerAdminPrivilege }: AuthModalProps) {
  ...
  const [loginInput, setLoginInput] = useState(''); // holds ユーザーID or Email
  const [email, setEmail] = useState(''); // holds register email
  const [regUserId, setRegUserId] = useState(''); // holds custom register User ID
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!loginInput.trim() || !password) {
      setError('ユーザーID（またはメールアドレス）とパスワードを入力してください。');
      return;
    }

    const users = getUsersFromStorage();
    const foundUser = users.find(u => 
      (u.id.toLowerCase() === loginInput.trim().toLowerCase() || u.email.toLowerCase() === loginInput.trim().toLowerCase()) && 
      u.password === password
    );
    ...
    if (foundUser) {
      ...
      if (nextFailCount >= 5) {
        ...
        setError(`ユーザーIDまたはパスワードが正しくありません。（セキュリティ保護: あと ${5 - nextFailCount} 回間違えるとロックされます）`);
      }
    }
  };
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    if (!regUserId.trim() || !name.trim() || !email.trim() || !password) {
      setError('すべての必須項目を入力してください。');
      return;
    }

    if (!/^[a-zA-Z0-9_-]{3,20}$/.test(regUserId.trim())) {
    ...
    const isDupId = users.some(u => u.id.toLowerCase() === regUserId.trim().toLowerCase());
    if (isDupId) {
      setError('このユーザーIDは既に登録されています。別のIDを指定してください。');
      return;
    }

    const newUser: UserType = {
      id: regUserId.trim().toLowerCase(),
      name: name.trim(),
        ...
        <div className="p-6">
          ...
          {activeTab === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">ユーザーID</label>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">ユーザーID（ログイン用）</label>
                <div className="relative font-sans">
                  ...
                  <input
                    ...
                    placeholder="半角英数字3〜20文字 (例: kanta123)"
                    className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-xl bg-gray-50/50 focus:outline-hidden font-medium text-gray-700 font-sans"
                  />
                </div>
                <p className="text-[10px] text-gray-400 mt-1 leading-relaxed">※ 2回目以降のログインはニックネームやメールアドレスではなく、この「ユーザーID」を使用します。</p>
              </div>
